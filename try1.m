...
[pks,locs] = findpeaks(signal_m_w_i(1 : 1 * fs),'MINPEAKDISTANCE',dist); %value and location

[pks1,locs1] = findpeaks(signal_m_w_i,'MINPEAKDISTANCE',dist)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
pks = []
steps = 0
for i = 1 : length(signal_m_w_i)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    a = signal_m_w_i(i)
    if i + 1 <= length(signal_m_w_i)
        b = signal_m_w_i(i+1)
    else 
        continue
    end
...
%peaks in the filtered signal
    if (locs(j) - N >= 1)
        [y_i, x_i] = max(signal_f_h(locs(j) - N : locs(j))); %high_pass filter signal
    else
        if (j==0)
            [y_i,x_i]=max(signal_f_h(1:locs(j))); %high_pass filter signal
        end
    end
    x_i = x_i + locs(j) - N - 1;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
    ...
    if ((length(qrs_position) ~= 0) && ((locs(j) - qrs_position(end)) >= round(1.66 * mean_RR2))) 

        %go back looking for peaks - max in that interval
        [pks_missed, locs_missed] = max(signal_m_w_i(qrs_position(end) + dist : locs(j) - dist));
    ...
    if (pks(j) > T1) 
        ...
        if ((length(qrs_position) ~= 0) && (locs(j) - qrs_position(end) <= upper_bound))
        ...
        if (T_check==0)
            ...
            if (y_i > T1_s) 
                %it is a QRS
                qrs_position = [qrs_position, locs(j)];
                qrs_value = [qrs_value, pks(j)];
                qrs_raw = [qrs_raw, x_i];
                SPK = 0.125 * pks(j) + 0.875 * SPK;
                SPK_s = 0.125 *y_i + 0.875 * SPK_s;
            else
                NPK = 0.125 * pks(j) + 0.875 * NPK;
